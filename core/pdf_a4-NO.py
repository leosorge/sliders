...
Sfondo navy, griglia teal, titoli verde, corpo panna.
...
def _logo(c):
    ...
    c.drawImage(img, W - 140, 5, width=130, height=130, mask="auto")
...
def _header(c, pub_title, date_str, fig_num):
    ...
    c.setFont(F_BOLD, 11)
    c.drawString(MARGIN, H - 35, pub_title.upper())
    c.setFont(F_REG, 9)
...
def _title_page(c, section, pub_title, date_str, footer_text, fig_num):
    ...
    for kind, text in other_items:
        if kind == "blank":
            ...
            for i, line in enumerate(_wrap("\u2022 " + text, F_REG, b_size, max_w - 12, c)):
