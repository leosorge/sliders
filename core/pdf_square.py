"""
core/pdf_square.py  —  Stile FT-CS Daily  (595 x 595 pt quadrato)
Stessa palette di pdf_a4.py, pagine quadrate.
...
def _ensure_fonts():
    global _FONTS_REGISTERED, F_REG, F_BOLD, F_ITAL
...
def _logo(c):
    ...
    c.drawImage(img, W - 118, 5, width=110, height=110, mask="auto")
...
def _header(c, pub_title, date_str, fig_num):
    ...
    c.setFont(F_BOLD, 10)
    c.drawString(MARGIN, H - 32, pub_title.upper())
    c.setFont(F_REG, 8)
    c.setFillColor(CREAM)
    c.drawRightString(W - MARGIN, H - 32, f"{date_str}  |  Fig. {fig_num}")
...
def _footer(c, footer_text):
    ...
    c.setFont(F_REG, 7)
    c.setFillColor(CREAM)
    c.drawString(MARGIN, 16, footer_text)
    c.restoreState()
...
def _title_page(c, section, pub_title, date_str, footer_text, fig_num):
    ...
    y = H - 72
    if bold_items:
        _, title_text = bold_items[0]
        c.setFillColor(GREEN)
        for line in _wrap(title_text, F_BOLD, t_size, max_w, c):
    ...
    for kind, text in other_items:
        if kind == "blank":
            ...
            for i, line in enumerate(_wrap("\u2022 " + text, F_REG, b_size, max_w - 10, c)):
                c.setFont(F_REG, b_size)
                c.setFillColor(CREAM)
                c.drawString(MARGIN + (10 if i > 0 else 0), y, line)
                y -= b_size * 1.4
            y -= 2
        else:
            c.setFillColor(CREAM)
...
def _content_page(c, section, pub_title, date_str, footer_text, fig_num):
    ...
    for kind, text in items:
        if kind == "blank":
            ...
            c.setFillColor(GREEN)
            for line in _wrap(text, F_BOLD, t_size, max_w, c):
                c.setFont(F_BOLD, t_size)
                c.drawString(MARGIN, y, line)
                y -= t_size * 1.3
            y -= 4
            ...
            for i, line in enumerate(_wrap("\u2022 " + text, F_REG, l_size, max_w - 10, c)):
