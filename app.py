...
# ── Tabs ──────────────────────────────────────────────────────────────────────
tab_genera, tab_testo, tab_lista, tab_viewer = st.tabs([
    "🚀 Genera da Google Docs",
    "📝 Genera da testo",
    "📋 Lista di txt",
    "📂 Viewer PDF A4",
])
...
# ═══════════════════════════════════════════════════════════════════════════════
# TAB 2 – Genera PDF da testo già formattato
# ═══════════════════════════════════════════════════════════════════════════════
with tab_testo:
    st.caption(
        "Incolla testo già formattato con i marcatori del sistema "
        "(`*grassetto*`, `_corsivo_`, `=` separatore) oppure carica uno o più file `.txt`. "
        "Il PDF viene generato direttamente, senza passare per Gemini."
    )

    with st.expander("📋 Formato atteso", expanded=False):
        st.markdown(
            "Ogni articolo deve seguire questa struttura:\n"
            "```\n"
            "*Titolo riga 1*\n"
            "*Titolo riga 2*\n"
            "=\n"
            ...
            "... (fino a 5 sezioni)\n"
            "```\n"
            f"I separatori `=` delimitano le sezioni. Ogni riga deve essere al massimo **{MAX_CHARS_A4} caratteri**."
        )

    input_mode = st.radio(
        "Modalità input",
        ["📁 Carica file .txt", "✏️ Incolla testo"],
        horizontal=True,
        label_visibility="collapsed",
    )

    testo_items: list[dict] = []

    if input_mode == "📁 Carica file .txt":
        uploaded_txts = st.file_uploader(
            "File .txt (uno per articolo)",
            type=["txt"],
        ...
        if uploaded_txts:
            for f in uploaded_txts:
                try:
                    ...
                    testo_items.append({"name": f.name.replace(".txt", ""), "text": content})
                except Exception as e:
                    st.error(f"Errore lettura {f.name}: {e}")
            if testo_items:
                st.success(f"✅ {len(testo_items)} file caricati.")
    else:
        nome_manuale = st.text_input(
            "Nome articolo (usato come nome file)",
            value="articolo_01",
            placeholder="es. unicenter_recap",
        )
        ...
        if testo_manuale.strip():
            testo_items = [{"name": nome_manuale.strip() or "articolo_01", "text": testo_manuale}]

    if not testo_items:
        st.info("Carica almeno un file .txt oppure incolla il testo per iniziare.")
    else:
        st.write(f"**{len(testo_items)} articolo/i** pronti per la generazione.")

        if st.button("📝 Genera PDF da testo", type="primary", key="btn_testo"):
            zip_a4_buf = BytesIO()
            zip_sq_buf = BytesIO()
            ...
            with zipfile.ZipFile(zip_a4_buf, "w", zipfile.ZIP_DEFLATED) as za, \
                ...
                for idx, item in enumerate(testo_items):
                    progress.progress(
                        idx / len(testo_items),
                        text=f"Articolo {idx + 1}/{len(testo_items)}: {item['name']}…",
                    )
                    try:
                        from core.gdocs import wrap_summary
                        summary = wrap_summary(item["text"].strip(), MAX_CHARS_A4)
                        fname   = item["name"]
                        ...
                        st.success(f"✅ {fname} — PDF generato (nessun token Gemini usato)")

                    except Exception as e:
                        st.error(f"❌ Errore su {item['name']}: {e}")
...
# ═══════════════════════════════════════════════════════════════════════════════
# TAB 3 – Lista di txt (link a file .txt già formattati)
# ═══════════════════════════════════════════════════════════════════════════════
with tab_lista:
    st.caption(
        "Incolla uno o più link a file `.txt` già formattati — Google Drive, GitHub raw, "
        "Dropbox o qualsiasi URL diretto. Il testo viene scaricato e convertito in PDF "
        "senza passare per Gemini."
    )
...
https://drive.google.com/file/d/1Cs_Kmkfa0VAEu0PdUFQ2hbQgFb_RMIbC/view?usp=drive_link"""
    ...
    urls_lista_raw = st.text_area(
        "Link ai file .txt (uno per riga)",
        value=_DEFAULT_LINKS,
        height=160,
        placeholder=(
            "https://drive.google.com/file/d/1-60Zy_KDMXl1jeTuxatMiOhUK2W3N-lS/view?usp=drive_link\n"
            "https://drive.google.com/file/d/1gN6JTuYqTb4SXaKqralUHST54h93b9pU/view?usp=drive_link\n"
            "https://raw.githubusercontent.com/utente/repo/main/articolo.txt"
        ),
        label_visibility="collapsed",
    )
    ...
    urls_lista = [
        u.strip()
        for u in urls_lista_raw.splitlines()
        if u.strip() and not u.strip().startswith("#")
    ]

    if not urls_lista:
        st.info("Incolla almeno un link per iniziare.")
    else:
        st.write(f"**{len(urls_lista)} link** da elaborare.")

        if st.button("📋 Genera PDF da lista", type="primary", key="btn_lista"):
            ...
            def _gdrive_direct(url: str) -> str:
                """Converte un link Google Drive /view in URL download diretto."""
                m = _re.search(r"/file/d/([a-zA-Z0-9_-]+)", url)
                if m:
                    return f"https://drive.google.com/uc?export=download&id={m.group(1)}"
                return url

            def _fetch_txt(url: str) -> tuple[str, str]:
                """Scarica un file txt e restituisce (nome, testo)."""
                download_url = _gdrive_direct(url)
                r = _req.get(download_url, headers=_HEADERS, timeout=20, allow_redirects=True)
                r.raise_for_status()
                try:
                    text = r.content.decode("utf-8")
                except UnicodeDecodeError:
                    text = r.content.decode("latin-1")
                # Nome: prova Content-Disposition, poi parte finale URL
                cd = r.headers.get("Content-Disposition", "")
                m_name = _re.search(r'filename[*]?=["\']?([^"\';]+)', cd)
                if m_name:
                    name = m_name.group(1).strip().replace(".txt", "")
                else:
                    # Usa l'ID Google Drive o la parte finale dell'URL originale
                    m_id = _re.search(r"/file/d/([a-zA-Z0-9_-]+)", url)
                    name = m_id.group(1)[:12] if m_id else url.rstrip("/").split("/")[-1].replace(".txt", "")
                return name, text
            ...
            with zipfile.ZipFile(zip_a4_buf, "w", zipfile.ZIP_DEFLATED) as za, \
                ...
                progress = st.progress(0, text="Scaricamento e generazione…")

                for idx, url in enumerate(urls_lista):
                    progress.progress(
                        idx / len(urls_lista),
                        text=f"Link {idx + 1}/{len(urls_lista)}: {url[:55]}…",
                    )
                    try:
                        with st.spinner(f"Link {idx + 1}: scaricamento…"):
                            name, raw_text = _fetch_txt(url)

                        from core.gdocs import wrap_summary
                        summary = wrap_summary(raw_text.strip(), MAX_CHARS_A4)

                        with st.spinner(f"Link {idx + 1}: PDF A4…"):
                            pdf_a4 = generate_pdf_a4(summary, pub_title, today_str, footer_text)

                        with st.spinner(f"Link {idx + 1}: PDF quadrato…"):
                            pdf_sq = generate_pdf_square(summary, pub_title, today_str, footer_text)

                        za.writestr(f"{name}_A4.pdf", pdf_a4)
                        zs.writestr(f"{name}_sq.pdf", pdf_sq)
                        _push_to_viewer(f"{name}_A4.pdf", pdf_a4)

                        st.success(f"✅ {name} ({len(raw_text):,} caratteri → PDF)")

                    except Exception as e:
                        st.error(f"❌ Link {idx + 1} ({url[:55]}): {e}")
