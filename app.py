...
# ── Tabs ──────────────────────────────────────────────────────────────────────
# ── Helper ────────────────────────────────────────────────────────────────────
def _pdf_names_changed(uploaded_files, stored: list) -> bool:
    """Ritorna True se i file caricati sono cambiati rispetto allo store."""
    uploaded_names = {f.name for f in uploaded_files}
    stored_names   = {e["name"] for e in stored}
    return uploaded_names != stored_names


tab_genera, tab_viewer = st.tabs(["🚀 Genera da Google Docs", "📂 Viewer PDF A4"])

# ═══════════════════════════════════════════════════════════════════════════════
# TAB 1 – Genera PDF da Google Docs  (codice originale invariato)
# ═══════════════════════════════════════════════════════════════════════════════
with tab_genera:
...
# ═══════════════════════════════════════════════════════════════════════════════
with tab_viewer:
    # ── Import lazy (evita crash se pymupdf non è installato) ─────────────────
    try:
    ...
    if not _viewer_ok:
        st.warning(
            "**PyMuPDF non trovato.** "
            "Aggiungi `pymupdf` a `requirements.txt` e rideploya l'app."
        )
        st.code("pymupdf>=1.23", language="text")
        st.stop()

    st.subheader("📂 Carica PDF A4")
    st.caption("Seleziona uno o più PDF A4 per visualizzarli come slider affiancati.")
    ...
    # ── Stato sessione ─────────────────────────────────────────────────────────
    # pdf_store: lista di dict {name, bytes, n_pages, current_page}
    if "pdf_store" not in st.session_state or _pdf_names_changed(uploaded, st.session_state.get("pdf_store", [])):
    ...
    # ── Outer slider (scorre gruppi da 3 PDF) ─────────────────────────────────
    VISIBLE = 3
    n_pdfs  = len(store)
    ...
    for col, idx in zip(cols, slots):
        ...
        with col:
            ...
            st.markdown(
                ...
                f"white-space:nowrap;' title='{entry['name']}'>"
                f"{entry['name']}</div>",
                unsafe_allow_html=True,
            )
            ...
            # Footer: navigazione interna pagine
            ...
            # Download pagina corrente come PNG
            st.download_button(
                label="⬇ PNG",
    ...
    with col_a:
        # ZIP con tutte le pagine come PNG
        if st.button("🗜 Crea ZIP pagine PNG", use_container_width=True):
            zip_buf = BytesIO()
    ...
    with col_b:
        # ZIP con i PDF originali
        if st.button("📄 ZIP PDF originali", use_container_width=True):
            zip_buf = BytesIO()
            with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
                for entry in store:
